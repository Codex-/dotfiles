...
-- no i dont comment my fucking lua
...
memory = lain.widgets.mem({
    settings = function()
        widget:set_markup(" " .. mem_now.used .. "<span color='" .. beautiful.sprt_color .. "'>MB </span>")
    end
})

cpu = lain.widgets.cpu({
    settings = function()
        widget:set_markup(" " .. cpu_now.usage .. "<span color='" .. beautiful.sprt_color .. "'>% </span>")
    end
})

temp = lain.widgets.temp({
    settings = function()
        widget:set_text(" CORE: " .. coretemp_now .. " <span color='" .. beautiful.sprt_color .. "'>°C </span>")
    end
})

time = awful.widget.textclock("%A <span color='" .. beautiful.sprt_color .. "'> %I:%M </span>", 60)
...
if wirelessw_enabled then 
wirelessTick:start()
end

spr = wibox.widget.textbox()
spr:set_markup("<span color='" .. beautiful.spr_color .. "'> II </span>")
sprs = wibox.widget.textbox()
sprs:set_markup("<span color'" .. beautiful.spr_color .. "'> I </span>")
...
for s = 1, screen.count() do
    ...
    bar[s] = awful.wibox({ position = "bottom", screen = s, height = 27})
...
awful.key({ modkey }, "b", function() 
    naughty.notify({ 
       text=string.format("%s%s", "It is currently:\n", "<span color='#ffffff'>" .. os.date ("%A %dth %b, %I:%M" .. string.lower(os.date("%p"))) .. "</span>"),
       timeout=2
    })
end),
...
awful.key({ modkey }, "v", function() 
    naughty.notify({
        icon_size=76,
        text=string.format("%s",  "<span color='#ffffff'>" .. string.sub(awful.util.pread("ncmpcpp --current-song"), 8, -1) .. "</span>"),
        timeout=3
    })
end),           
...
awful.util.spawn("dmenu_run -i -fn 'Open Sans-8' -h '26 '  -p 'Run command:' -nb '" .. 
